...
function initTrustSphere() {
    ...
    // Gate nav links forward (robust: capture all links globally)
    document.addEventListener('click', function(e) {
        ...
        const wantsModule2 = /(?:^|\/)module2\.html(?:[?#].*)?$/i.test(href) || href.toLowerCase() === 'module2.html';
        const wantsModule3 = /(?:^|\/)module3\.html(?:[?#].*)?$/i.test(href) || href.toLowerCase() === 'module3.html';
        if (wantsModule2 && !progress.m1) {
            e.preventDefault();
            alert('Complete Module 1 first.');
        } else if (wantsModule3 && !(progress.m1 && progress.m2)) {
            e.preventDefault();
            alert('Complete Modules 1 and 2 first.');
        }
    }, true);

    // If user lands directly on locked module pages, redirect them back
    (function enforceModuleAccessOnLoad(){
        const path = (location.pathname || '').toLowerCase();
        const progress = getProgress();
        if (path.endsWith('/module2.html') && !progress.m1) {
            alert('Complete Module 1 first.');
            location.replace('module1.html');
        } else if (path.endsWith('/module3.html') && !(progress.m1 && progress.m2)) {
            alert('Complete Modules 1 and 2 first.');
            location.replace('module2.html');
        }
    })();
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initTrustSphere);
} else {
    // DOM is already ready (e.g., script at end of body or cached load)
    initTrustSphere();
}
